...
hello:
	@echo ""
	@echo "${g}${bd}MadBot command line interface${n}${st}"
	@echo ""
...
update: hello
	...
	( echo "${r}[!] You are not using version control system (git). We recommend to use git. MadBot.py will be updated and backed up.${n}" && \
	while [ -z "$$CONTINUE" ]; do \
		read -r -p "Are you sure you wish to continue? [y/N] " CONTINUE; \
	done ; \
	( test $$CONTINUE = "y" || test $$CONTINUE = "Y" ) && \
	( mv MadBot.py MadBot.py.backup && wget https://raw.githubusercontent.com/cusdt-LINK/alpha-kcc-dex-bot/master/MadBot.py -O MadBot.py && wget https://raw.githubusercontent.com/cusdt-LINK/alpha-kcc-dex-bot/master/requirements.txt -O requirements.txt ) || \
...
run: hello prepare_venv check prepare_args 
	...
	@${PYTHON} MadBot.py -s $(settings) -t $(tokens) ${args}
...
%_start: 
	...
	-v $(HOME)/MadBot.py:/app/MadBot.py -v $(HOME)/instances/$*/logs/:/app/logs/ \
	--entrypoint '/bin/bash' alpha_bot -c 'python MadBot.py ${args}')
