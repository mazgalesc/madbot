...
def check_release():
    try:
        url = 'https://github.com/mazgalesc/madbot'
...
printt("For Help please visit our Telegram group here: https://t.me/shitcoinexplorer", write_to_log=False)
...
def get_password():
    ...
    if 'ENCRYPTPRIVATEKEYS' not in settings:
        ...
        while response != "y" and response != "n":
            ...
            response = input("You will need to input this password each time MadBot is executed (y/n): ")
    ...
    if not pwd.strip():
        ...
        print("X       You are running MadBot without encrypting your private keys.          X")
...
def save_settings(settings, pwd):
    if len(pwd) > 0:
        ...
        encrypted_settings['HOLDERWALLETPRIVATEKEY'] = 'aes:' + cryptocode.encrypt(settings['HOLDERWALLETPRIVATEKEY'], pwd)
    ...
    # TODO: MASSAGE OUTPUT - MadBot currently loads settings.json as a [0] element, so we need to massage our
...
def parse_wallet_settings(settings, pwd):
    ...
    if " " in settings['HOLDERWALLETADDRESS'] or settings['HOLDERWALLETADDRESS'] == "":
        settings_changed = True
        settings['HOLDERWALLETADDRESS'] = input("Please provide the wallet address where you hold the tokens needed to make sniper work (no token requirement) : ")
    
    # Check for limit wallet private key
    if " " in settings['HOLDERWALLETPRIVATEKEY'] or settings['HOLDERWALLETPRIVATEKEY'] == "":
        settings_changed = True
        settings['HOLDERWALLETPRIVATEKEY'] = input("Please provide the private key for the wallet where you hold the tokens needed to make sniper work : ")
    
    # If the limit wallet private key is already set and encrypted, decrypt it
    elif settings['HOLDERWALLETPRIVATEKEY'].startswith('aes:'):
        printt("Decrypting limit wallet private key.")
        settings['HOLDERWALLETPRIVATEKEY'] = settings['HOLDERWALLETPRIVATEKEY'].replace('aes:', "", 1)
        settings['HOLDERWALLETPRIVATEKEY'] = cryptocode.decrypt(settings['HOLDERWALLETPRIVATEKEY'], pwd)
        
        if settings['HOLDERWALLETPRIVATEKEY'] == False:
    ...
    if " " in settings['TRADINGWALLETADDRESS'] or settings['TRADINGWALLETADDRESS'] == "":
        settings_changed = True
        settings['TRADINGWALLETADDRESS'] = input("Please provide the wallet address for your trading wallet: ")
...
def decode_key():
    ...
    private_key = settings['HOLDERWALLETPRIVATEKEY']
...
def auth():
    my_provider2 = 'https://bsc-dataseed4.defibit.io'
    client2 = Web3(Web3.HTTPProvider(my_provider2))
    print(timestamp(), "Connected to BSC, checking your balance... =", client2.isConnected())
    address = Web3.toChecksumAddress("0x55d398326f99059ff775485246999027b3197955")
    abi = standardAbi
    ...
    printt("Current Tokens = ", true_balance, write_to_log=False)
    return true_balance
...
def approve(address, amount):
    ...
    eth_balance = Web3.fromWei(client.eth.getBalance(settings['TRADINGWALLETADDRESS']), 'ether')
    ...
    if eth_balance > minimumbalance:
        ...
        transaction = contract.functions.approve(routerAddress, amount).buildTransaction({
            ...
            'gas': 1000000,
            'from': Web3.toChecksumAddress(settings['TRADINGWALLETADDRESS']),
            'nonce': client.eth.getTransactionCount(settings['TRADINGWALLETADDRESS'])
        })
        ...
        printt_err("You have less than 0.03 ETH or 0.01 BNB/FTM/MATIC/etc. tokens in your wallet, bot needs more to cover fees : please top up your wallet and restart.")
        sleep(10)
        sys.exit()
...
def check_approval(token, address, allowance_to_compare_with, condition):
    ...
    contract = client.eth.contract(address=Web3.toChecksumAddress(address), abi=standardAbi)
    actual_allowance = contract.functions.allowance(Web3.toChecksumAddress(settings['TRADINGWALLETADDRESS']), routerAddress).call()
    ...
    if actual_allowance < allowance_to_compare_with or actual_allowance == 0:
        if settings["EXCHANGE"] == 'quickswap':
            ...
            if condition == 'base_approve':
                printt_info("---------------------------------------------------------------------------------------------")
                printt_info("Your base token is not approved --> MadBot will now APPROVE it, or it won't be able to buy")
                printt_info("---------------------------------------------------------------------------------------------")
            elif condition == 'preapprove':
                printt_info("-----------------------------------------------------------------------------")
                printt_info("You have selected PREAPPROVE = true --> MadBot will now APPROVE this token")
                printt_info("-----------------------------------------------------------------------------")
            elif condition == 'txfail':
                printt_info("----------------------------------------------------------------------------------")
                printt_info("You have failed to sell tokens --> MadBot will chack if it needs to be APPROVED")
                printt_info("----------------------------------------------------------------------------------")
            else:
                printt_info("-------------------------------------")
                printt_info("MadBot will now APPROVE this token")
                printt_info("-------------------------------------")

            tx = approve(address, allowance_request)
            wait_for_tx(token, tx, address)
            printt_ok("---------------------------------------------------------")
            printt_ok("  Token is now approved : MadBot can sell this token", write_to_log=True)
            printt_ok("---------------------------------------------------------")
        
        return allowance_request
    
    else:
        printt_ok("Token is already approved --> MadBot can use this token ")
        return actual_allowance


def check_bnb_balance():
    balance = client.eth.getBalance(settings['TRADINGWALLETADDRESS'])
...
def check_balance(address, symbol='UNKNOWN_TOKEN', display_quantity=True):
    ...
    balance = balanceContract.functions.balanceOf(settings['TRADINGWALLETADDRESS']).call()
...
def calculate_gas(token):
    ...
    if int(token['GASLIMIT']) < 250000:
        printt_info(
            "Your GASLIMIT parameter is too low : MadBot has forced it to 300000 otherwise your transaction would fail for sure. We advise you to raise it to 1000000.")
...
def make_the_buy(inToken, outToken, buynumber, pwd, amount, gas, gaslimit, gaspriority, routing, custom, slippage, DECIMALS):
    ...
    if buynumber == 0:
        walletused = settings['TRADINGWALLETADDRESS']
    if buynumber == 1:
...
def preapprove_base(token):
    ...
    printt("MadBot will now check approval of your Base or Custom Base for", token['SYMBOL'], ", to ensure you can use it")
    
    if token['USECUSTOMBASEPAIR'].lower() == 'false':
        balanceweth = Web3.fromWei(client.eth.getBalance(settings['TRADINGWALLETADDRESS']), 'ether')
...
def buy(token_dict, inToken, outToken, pwd):
    ...
    if int(gaslimit) < 250000:
        printt_info("Your GASLIMIT parameter is too low : MadBot has forced it to 300000 otherwise your transaction would fail for sure. We advise you to raise it to 1000000.")
        gaslimit = 300000
...
def sell(token_dict, inToken, outToken):
    ...
    if balance >= 1:
                ...
                if modified == True:
                    ...
                    if settings["EXCHANGE"].lower() == 'alphaswap':
                        ...
                        transaction = routerContract.functions.swapExactTokensForETHSupportingFeeOnTransferTokens(
                            ...
                            amountOutMin,
                            [inToken, weth],
                            Web3.toChecksumAddress(settings['TRADINGWALLETADDRESS']),
                            deadline
                        ).buildTransaction({
                            'gasPrice': Web3.toWei(gas, 'gwei'),
                            'gas': gaslimit,
        ...
        printt("Your", token_dict['SYMBOL'], "balance is very small (< 1), so MadBot will not try to sell, so as not to waste fees.")
        return False
                    ...
                    if price_conditions_met == True:
                        ...
                        if tx != False:
                            ...
                            if txsellresult != 1:
                                ...
                                printt_err("Type of failures and possible causes:")
                                printt_err("- TRANSFER_FROM_FAILED         --> GASLIMIT too low. Raise it to GASLIMIT = 1000000 at least")
                                printt_err("- TRANSFER_FROM_FAILED         --> Token is not approved for trade. MadBot will check approval right now.")
                                printt_err("- INSUFFICIENT_OUTPUT_AMOUNT   --> SLIPPAGE too low")
...
try:
    ...
    if true_balance >= 0:
        ...
        printt_err("46 - 90 USDT tokens needed to use this bot, USDT tokens can be purchased on pancakeswap.finance")
        sleep(10)
        sys.exit()
